...
func WarnUnknownAICEnv() {
    ...
    unused := map[string]string{
        EnvAICProvider: "reserved; not currently used",
    }
    printedHeader := false
    printHdr := func() {
    ...
    for _, entry := range os.Environ() {
        // entry is KEY=VALUE
        if !strings.HasPrefix(entry, "AIC_") { continue }
        k := entry
        if i := strings.IndexByte(entry, '='); i >= 0 { k = entry[:i] }
        if _, ok := known[k]; ok { continue }
