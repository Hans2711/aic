...
func TestComposeUserContentWithSummary(t *testing.T) {
	...
	if len(uc) == 0 { t.Fatal("user content empty") }
	...
	if ucFind := findSegment(uc, trunc[:200]); ucFind == -1 { t.Fatalf("truncated diff segment not found in user content") }
}

func TestComposeUserContentNoSummary(t *testing.T) {
	full := makeStr(1000)
	uc := composeUserContent(full, full, "")
	if uc != full { t.Fatalf("expected raw trunc when no summary, got length %d vs %d", len(uc), len(full)) }
}

func makeStr(n int) string {
	b := make([]byte, n)
	for i := 0; i < n; i++ { b[i] = byte('a' + (i % 26)) }
	return string(b)
}

func containsAll(s string, subs []string) bool {
	for _, sub := range subs { if !strings.Contains(s, sub) { return false } }
	return true
}
