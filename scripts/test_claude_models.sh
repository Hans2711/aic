...
# Sanity-check that this aic mentions Claude support
if ! aic --help 2>/dev/null | grep -qi "CLAUDE_API_KEY"; then
  echo "The 'aic' on PATH may be outdated and not support Claude. Ensure you are using the freshly built binary for your platform in dist/." >&2
  exit 1
fi
...
for m in $MODELS; do
  ...
  git add "$TEST_FILE" 2>/dev/null || true
  if ! (env -i PATH="$PATH" AIC_PROVIDER=claude AIC_MODEL="$m" AIC_SUGGESTIONS="$AIC_SUGGESTIONS" CLAUDE_API_KEY="$CLAUDE_API_KEY" aic <<< $'1\n n\n'); then
